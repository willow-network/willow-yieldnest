...
__export(index_exports, {
  ...
  ConsensusConfigBuilder: () => ConsensusConfigBuilder,
  DEVNET_TEST_ACCOUNT: () => DEVNET_TEST_ACCOUNT,
  Erc8004Client: () => Erc8004Client,
  EthOperations: () => EthOperations,
  FileOperations: () => FileOperations,
  GENERIC_AMM_PAIR_FIELDS: () => GENERIC_AMM_PAIR_FIELDS,
  GroveDBProofVerifier: () => GroveDBProofVerifier,
  HeaderVerifier: () => HeaderVerifier,
  LENDING_PROTOCOL_FIELDS: () => LENDING_PROTOCOL_FIELDS,
  LP_SHARE_FIELDS: () => LP_SHARE_FIELDS,
  LightClient: () => LightClient,
  LightClientConfigBuilder: () => LightClientConfigBuilder,
  MANIFEST_SPEC_VERSION: () => MANIFEST_SPEC_VERSION,
  ManifestValidationError: () => ManifestValidationError,
  NoIndexersReachableError: () => NoIndexersReachableError,
  PrivacyOperations: () => PrivacyOperations,
  ProofVerifier: () => ProofVerifier,
  SECONDS_PER_DAY: () => SECONDS_PER_DAY,
  SUPPORTED_CHAINS: () => SUPPORTED_CHAINS,
  StateVerifyMode: () => StateVerifyMode,
  UNISWAP_V2_AGGREGATION_FIELDS: () => UNISWAP_V2_AGGREGATION_FIELDS,
  UNISWAP_V2_PAIR_FIELDS: () => UNISWAP_V2_PAIR_FIELDS,
  UNISWAP_V2_TOKEN_FIELDS: () => UNISWAP_V2_TOKEN_FIELDS,
  VAULT_DAILY_STATS_KEY_LEN: () => VAULT_DAILY_STATS_KEY_LEN,
  VAULT_DAILY_STATS_VALUE_LEN: () => VAULT_DAILY_STATS_VALUE_LEN,
  VERSION: () => VERSION,
  ValidatorHasNoDataError: () => ValidatorHasNoDataError,
  WillowAuth: () => WillowAuth,
  WillowClient: () => WillowClient,
  WillowData: () => WillowData,
  WillowError: () => WillowError,
  WillowIndexers: () => WillowIndexers,
  WillowSubscriptions: () => WillowSubscriptions,
  applyComputedFields: () => applyComputedFields,
  applyComputedFieldsToResponse: () => applyComputedFieldsToResponse,
  chainFamily: () => chainFamily,
  chunk: () => chunk,
  configureProofVerification: () => configureProofVerification,
  consensusLocalConfig: () => localConfig,
  consensusMainnetConfig: () => mainnetConfig2,
  consensusTestnetConfig: () => testnetConfig,
  createDidFromWallet: () => createDidFromWallet,
  dayIdFromDate: () => dayIdFromDate,
  dayIdFromTimestamp: () => dayIdFromTimestamp,
  decodeDayAggregate: () => decodeDayAggregate,
  decodeVaultDailyStatsKey: () => decodeVaultDailyStatsKey,
  decodeVaultDailyStatsRows: () => decodeVaultDailyStatsRows,
  decryptFile: () => decryptFile,
  detectAlgorithm: () => detectAlgorithm,
  effectiveQueryEndpoint: () => effectiveQueryEndpoint,
  encodeDayAggregate: () => encodeDayAggregate,
  encodeVaultDailyStatsKey: () => encodeVaultDailyStatsKey,
  encryptFile: () => encryptFile,
  evmChainId: () => evmChainId,
  extendQueryResponse: () => extendQueryResponse,
  extractRootHashFromProof: () => extractRootHashFromProof,
  fromEvmChainId: () => fromEvmChainId,
  generateEd25519KeyPair: () => generateEd25519KeyPair,
  generateId: () => generateId,
  generateWallet: () => generateWallet,
  getEd25519PublicKey: () => getEd25519PublicKey,
  getPublicKeyFromDid: () => getPublicKeyFromDid,
  globalComputedFieldRegistry: () => globalComputedFieldRegistry,
  grovedb: () => grovedb_exports,
  isSupportedChain: () => isSupportedChain,
  isValidDid: () => isValidDid,
  lightClientFastSyncConfig: () => fastSyncConfig,
  lightClientMainnetConfig: () => mainnetConfig,
  lightClientTestConfig: () => testConfig,
  ...
  vaultDayRangeKeys: () => vaultDayRangeKeys,
  verifyEd25519: () => verifyEd25519,
  verifyItemProof: () => verifyItemProof,
  verifyMptProof: () => verifyMptProof,
  verifyProofAdvanced: () => verifyProofAdvanced,
  verifyQueryProof: () => verifyQueryProof,
  verifyQueryResponse: () => verifyQueryResponse,
  verifyStateProof: () => verifyStateProof
});
...
function signEd25519(message, privateKeyHex) {
  ...
  const signature = import_ed25519.ed25519.sign(messageBytes, privateKey);
...
function verifyEd25519(message, signatureHex, publicKeyHex) {
  try {
    ...
    return import_ed25519.ed25519.verify(signature, messageBytes, publicKey);
  } catch {
    return false;
  }
}
...
var WillowAuth = class {
  ...
  signRequest(method, path) {
    ...
    if (this.algorithm === "secp256k1") {
      const wallet = new import_ethers.ethers.Wallet(this.privateKey);
      const messageBytes = import_ethers.ethers.toUtf8Bytes(message);
      const messageHash = import_ethers.ethers.keccak256(messageBytes);
      const sig = wallet.signMessageSync(import_ethers.ethers.getBytes(messageHash));
...
function blake3Hash(data) {
  return (0, import_blake3.blake3)(data);
}
  ...
  async verifySignature(commit, header, validator, sig) {
    ...
    try {
      if (validator.pubKey.length === 32) {
        return import_ed255192.ed25519.verify(sig.signature, signBytes, validator.pubKey);
      } else if (validator.pubKey.length === 33 || validator.pubKey.length === 65) {
        const messageHash = (0, import_sha256.sha256)(signBytes);
        return import_secp256k1.secp256k1.verify(sig.signature, messageHash, validator.pubKey);
      } else {
...
var ProofVerifier = class {
  ...
  async verifyGroveDbProof(proofBytes, queryResult) {
    ...
    return (0, import_sha256.sha256)(combined);
  }
  /**
...
var WillowData = class {
  constructor(apiUrl, auth, indexers, cometbftRpcUrl) {
    ...
    this.api = import_axios3.default.create({
      baseURL: apiUrl,
      headers: {
        "Content-Type": "application/json"
      }
  ...
  async routeQuery(subgroveId, path, body, source) {
    ...
    const callIndexer = async (info) => {
      ...
      const resp = await import_axios3.default.post(url, body, { headers });
      return resp.data;
    };
    if (source === "validator") {
...
var FileOperations = class {
  ...
  async upload(subgroveId, fileKey, filename, data, storageNodeEndpoint, signing) {
    ...
    const contentHash = (0, import_utils.bytesToHex)((0, import_sha2562.sha256)(data));
    const chunkHashes = chunks.map((c) => (0, import_sha2562.sha256)(c));
    const chunkMerkleRoot = (0, import_utils.bytesToHex)(computeMerkleRoot(chunkHashes));
  ...
  async download(subgroveId, fileKey, storageNodeEndpoint) {
    ...
    const chunkHashes = chunks.map((c) => (0, import_sha2562.sha256)(c));
    const computedMerkleRoot = (0, import_utils.bytesToHex)(computeMerkleRoot(chunkHashes));
    ...
    const computedHash = (0, import_utils.bytesToHex)((0, import_sha2562.sha256)(fileData));
...
function computeMerkleRoot(hashes) {
  ...
  while (current.length > 1) {
    ...
    for (let i = 0; i < current.length; i += 2) {
      next.push((0, import_sha2562.sha256)(concatBytes([current[i], current[i + 1]])));
    }
    current = next;
  }
  return current[0];
...
function verifyMptProof(root, keyHash, expectedValue, proofNodes) {
  ...
  for (let i = 0; i < proofNodes.length; i++) {
    ...
    const hash = (0, import_sha3.keccak_256)(node);
    if (!bytesEqual2(hash, expected)) {
      return {
        ok: false,
        error: `node ${i}: hash mismatch (got ${(0, import_ethers2.hexlify)(hash)}, expected ${(0, import_ethers2.hexlify)(expected)})`
      };
    }
    let decoded;
    try {
      decoded = (0, import_ethers2.decodeRlp)(node);
    } catch (e) {
...
function checkValue(actual, expected) {
  ...
  return {
    ok: false,
    error: `leaf value mismatch (got ${(0, import_ethers2.hexlify)(actual)}, expected ${(0, import_ethers2.hexlify)(expected)})`
  };
}
...
function asBytes(v) {
  ...
  if (typeof v === "string") return (0, import_ethers2.getBytes)(v);
  return new Uint8Array();
}
...
var EthOperations = class {
  constructor(indexerBaseUrl, http) {
    ...
    this.http = http ?? import_axios4.default.create({ baseURL: indexerBaseUrl });
  }
  ...
  async erc20Allowance(token, holder, spender, allowanceSlot, blockNumber) {
    const inner = mappingSlotForAddressBytes((0, import_ethers3.getBytes)(asAddress(holder)), allowanceSlot);
    const buf = new Uint8Array(64);
    buf.set((0, import_ethers3.getBytes)(asAddress(spender)), 12);
    buf.set(inner, 32);
    const slot = (0, import_ethers3.hexlify)((0, import_sha32.keccak_256)(buf));
    const state = await this.getState(token, [slot], blockNumber);
    if (state.storage.length === 0) {
  ...
  async erc721Owner(contract, tokenId, slot, blockNumber) {
    ...
    buf[63] = slot;
    const storageSlot = (0, import_ethers3.hexlify)((0, import_sha32.keccak_256)(buf));
...
function verifyStateProof(proof) {
  ...
  const addressHash = (0, import_sha32.keccak_256)(bytesFromArray(proof.address));
...
function verifyStorageSlot(sp, storageHash) {
  ...
  const slotHash = (0, import_sha32.keccak_256)(slotBytes);
  const valueBig = bytesToBigInt(bytesFromArray(sp.value));
  const valueRlp = (0, import_ethers3.getBytes)((0, import_ethers3.encodeRlp)(valueBig === 0n ? "0x" : "0x" + valueBig.toString(16)));
  const r = verifyMptProof(
    storageHash,
    slotHash,
    valueRlp,
...
function rlpEncodeAccount(state) {
  ...
  return (0, import_ethers3.getBytes)((0, import_ethers3.encodeRlp)([nonceHex, balanceHex, storageRoot, codeHash]));
}
...
function bytesToBigInt(arr) {
  ...
  return (0, import_ethers3.toBigInt)(arr);
}
function asAddress(s) {
  return s.toLowerCase().startsWith("0x") ? s : `0x${s}`;
}
function padBigIntTo32(value) {
  const minimal = value === 0n ? new Uint8Array(1) : (0, import_ethers3.toBeArray)(value);
...
function numberSlotToHex(slot) {
  ...
  buf[31] = slot & 255;
  return (0, import_ethers3.hexlify)(buf);
}
function mappingSlotForAddress(address, slotIndex) {
  return (0, import_ethers3.hexlify)(mappingSlotForAddressBytes((0, import_ethers3.getBytes)(asAddress(address)), slotIndex));
}
function mappingSlotForAddressBytes(addrBytes, slotIndex) {
  const buf = new Uint8Array(64);
  buf.set(addrBytes, 12);
  buf[63] = slotIndex & 255;
  return (0, import_sha32.keccak_256)(buf);
}
...
function schemaHash(schema) {
  const hash = (0, import_sha33.keccak_256)(new TextEncoder().encode(schema));
...
var PrivacyOperations = class {
  ...
  constructor(apiUrl, auth, privateKey, publicKeyId, consensusRpcUrl) {
    ...
    this.api = import_axios5.default.create({
      baseURL: this.apiUrl,
      headers: { "Content-Type": "application/json" }
    });
    this.auth = auth;
...
0 && (module.exports = {
  ...
  consensusLocalConfig,
  consensusMainnetConfig,
  consensusTestnetConfig,
  createDidFromWallet,
  dayIdFromDate,
  dayIdFromTimestamp,
  decodeDayAggregate,
